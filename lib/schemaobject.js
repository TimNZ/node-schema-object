    ...
    return function (defaults)
    {
        ...
        this.toObject = function (options)
        {
            ...
            options = _.defaults({}, options, { all: null, 
                alias: false, undefineds: null, nulls: null, changed: null,
            ...
            _.each(schema, function (properties, index)
            {
                ...
                if (options.all !== true && (properties.invisible || (properties.toObject == 'never' && options.nevers === false)
                    || (properties.toObject == 'changed' && (options.changed === false || !self.tracking.hasChanged(realIndex)))
                    || (properties.type == 'alias' && !options.alias)
                    ...
                    || (properties.toObject == 'never' && options.never !== false)
                    || (properties.toObject == 'hasValue' && ((_.isNull(self[index]) && !options.nulls) || options.hasValues === false))) )
                {
                    return;
                }
