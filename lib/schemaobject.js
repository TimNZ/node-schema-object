    ...
    return function (defaults)
    {
        ...
        this.validate = function(options,parentPath)
        {
            ...
            _.each(schema, function (properties, index)
            {
                ...
                if (properties.type == 'object') {
                    error = value.validate(options);
                }
                else if (properties.type == 'array') {
                    _.each(value, function(arrayItem)
                    {
                        if (_.isObject(arrayItem) && _.isFunction(arrayItem.validate))
                            error = arrayItem.validate(options, (parentPath && '.') + properties.index);
                        else if (properties.validators)
                            error = validateItem(arrayItem, properties.validators);
                    })
                }
                else if (properties.validators)
                    error = validateItem(value,  properties.validators);
                if (_.isArray(error)
                    errors.concat(error);
            ...
            function validateItem(value, validators)
            {
            }
